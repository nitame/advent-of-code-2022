    ...
    if let Ok(lines) = read_lines(file_path) {
        for line in lines {
            if let Ok(line) = line {
                ...
                if section_overlap(&left_elf_section_bounds, &right_elf_section_bounds) {
                    total = total + 1
                }
            }
        }
...
fn section_overlap(left_section: &Vec<&str>, right_section: &Vec<&str>) -> bool {
    let left_start_bound: i32 = left_section[0].parse().unwrap();
    let left_end_bound: i32 = left_section[1].parse().unwrap();
    let right_start_bound: i32 = right_section[0].parse().unwrap();
    let right_end_bound: i32 = right_section[1].parse().unwrap();

    let left_range = left_start_bound..=left_end_bound;
    let right_range = right_start_bound..=right_end_bound;

    (left_range.contains(&right_start_bound) || left_range.contains(&right_end_bound))
        || (right_range.contains(&left_start_bound) || right_range.contains(&left_end_bound))
}
